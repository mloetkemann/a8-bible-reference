...
describe('bible', async () => {
  ...
  it('find book', async () => {
    ...
    assert.equal(bible.findBook('1. Mose')?.getName(), '1. Mose')
  ...
  it('map to json', async () => {
    ...
    assert.equal(
      ...
      '{"book":1,"chapter":3,"verse":1,"toVerse":null,"language":"de","translation":"de_slt","readableString":"2. Mose 3, 1"}',
    )
  })
  ...
  it('parsing passage verse', async () => {
    ...
    await assertBibleParser('1. Mose 1, 4')
    await assertBibleParser('Josua 4, 3')
  })

  it('parsing passage chapter only', async () => {
    await assertBibleParser('2. Mose 1')
    await assertBibleParser('4. Mose 4')
    await assertBibleParser('Offenbarung 4')
  })
  ...
  it('parsing passage wrong book', async () => {
    await assertParserFailed('ABC 1, 1')
    await assertParserFailed('mosa 1, 1')
    await assertParserFailed('OffP 1, 1')
  })
